...
# bbdsInstaller.sh
# Robust folder/project initializer with logging, version check, nvm setup

# -------- User Variables ---------
NODE_VERSION=20    # Set the desired Node.js major version here
...
log_msg() {
  ...
  echo -e "[$now] [$status] $msg" | tee -a "$LOGFILE"
}
...
check_curl() {
  if ! command -v curl >/dev/null 2>&1; then
    ...
    echo "Debian/Ubuntu: sudo apt-get update && sudo apt-get install curl"
    echo "Fedora/CentOS: sudo dnf install curl"
